...
def button_clicked():
    ...
    result = f_to_c(int(num))

    new_text = f"%sF = %ic"%(num , result)
    lbl_display.config (text = new_text)
...
lbl_fahrenheit =Label(root, text= "Fahrenheit").grid(row= 0, column= 0)
entry= Entry( root, textvariable= convert).grid (row= 0, column= 1)
button = Button (root, text= "Convert", command= button_clicked).grid (row= 1, column= 0, columnspan= 2)
lbl_display = Label (root, text= "")
lbl_display.grid (row= 2, column= 0, columnspan= 2)
